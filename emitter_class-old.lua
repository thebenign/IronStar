...
local part_img = love.graphics.newImage("/img/particle02.tga")
emitter.part_batch = love.graphics.newSpriteBatch(part_img,50000,"static")
...
function emitter.new(...)
  ...
  self.emit_t = 1    -- timer
  self.part_t = 30    -- ?
  self.pps = 1    -- particles per second?
  self.a = 0    -- angle in radians
  self.spread = 360    -- angle variation in radians
  self.s = 0    -- pixels per step movement
  self.acc_a = 0    -- acceleration angle in radians
  self.acc_s = .5    -- acceleration speed
  self.tang_d = 1    -- tangental angle delta?
...
function emitter:draw()
  local cx, cy = camera.x or 0, camera.y or 0 -- grab the camera coords or else just use the top-left corner of the screen
  
  local rr,rg,rb,ra = love.graphics.getColor()  -- grab the current brush color

  love.graphics.setBlendMode("add")  -- pretty coloooors
  local c = self.color
